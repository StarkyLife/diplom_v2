...
uses
  ...
  Menus, ImgList, ConnectPage, Errorpage, AddPage;
...
begin
  if (EditorMode=True) then MainForm.SaveTable;
  result:=inherited SelectCell(ACol, ARow);
end;

procedure TStringGrid.KeyPress(var Key: Char);
begin
  if key=#13 then MainForm.SaveTable;
  inherited;
end;
    ...
    begin
      with StringGridMain do
        begin
          if (rowCount > 1) and (row > 0) then
          begin
              modEntry := ADEntry.Create(Cells[0, Row]);
              modEntry.AddAttribute('isDefunct', ['FALSE']);
              status := schema.ModifyEntryAttributes(modEntry);
              modEntry.Destroy;
              modEntry := nil;
            ...
            if TreeViewCurrentNode.Selected then
            begin
              modEntry := ADEntry.Create(TreeViewCurrentNode.Text);
              modEntry.AddAttribute('isDefunct', ['FALSE']);
              status := schema.ModifyEntryAttributes(modEntry);
              modEntry.Destroy;
              modEntry := nil;
...
procedure TMainForm.Deactivate1Click(Sender: TObject);
var status : ADSchemaStatus;
  modEntry : ADEntry;
begin
  if schema = nil then
    ...
    begin
      with StringGridMain do
        begin
          ...
          begin
              modEntry := ADEntry.Create(Cells[0, Row]);
              modEntry.AddAttribute('isDefunct', ['TRUE']);
              status := schema.ModifyEntryAttributes(modEntry);
              modEntry.Destroy;
              modEntry := nil;
              if status.StatusType <> SuccessStatus then
...
begin
  ...
  begin
    StringGridMain.ColWidths[1] := trunc(StringGridMain.Width*0.67);
  end;
  if StringGridMain.ColCount = 3 then
  begin
    StringGridMain.ColWidths[1] := trunc(StringGridMain.Width*0.37);
...
begin
  ...
  StringGridMain.Selection := TGridRect(Rect(-1,-1,-1,-1));
...
begin
  ...
  pvResizeGridColumns;
  StringGridMain.Options := StringGridMain.Options+[goEditing];
  StringGridMain.OnSelectCell := StringGridMainSelectCellWithEdit;
...
begin
  with StringGridMain do
  begin
    if Cells[0,3] = 'cn' then
    begin
      attrValue := Cells[Col, Row];
      attrName := Cells[0, Row];
      entryName := Cells[1,3];
      modEntry := ADEntry.Create(entryName);
      attrIndex := modEntry.AddAttribute(attrName, [attrValue]);

      if Cells[0, Row-1] = attrName then
      begin
        whileRow := Row-1;
        while Cells[0, whileRow] = attrName  do
        begin
          modEntry.Attributes[attrIndex].AddValue(Cells[1, whileRow]);
          whileRow := whileRow - 1;
          if whileRow <= 0 then
            break;
        end;
      end;
      if (Row+1) < (RowCount-1) then
        if Cells[0, Row + 1] = attrName then
        begin
          whileRow := Row+1;
          while Cells[0, whileRow] = attrName  do
